...
export function ModelReaderToc({ model, containerRef }: ModelReaderTocProps) {
  ...
  const items: TocItem[] = [
    ...
    ...model.business_rules.map(r => ({ id: `rule-${r.id}`, label: r.id, section: 'business_rules' })),
    ...model.constraints.map(c => ({ id: `constraint-${c.id}`, label: c.id, section: 'constraints' })),
    ...model.open_questions.map(q => ({ id: `question-${q.id}`, label: q.id, section: 'open_questions' })),
  ]
